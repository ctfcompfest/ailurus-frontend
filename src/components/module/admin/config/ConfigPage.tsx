...
export default function ConfigPage() {
  ...
  const CONFIG_GROUPS: Record<string, string[]> = {
    ...
    "Contest Progress": ["CURRENT_TICK", "CURRENT_ROUND", "LAST_TICK_CHANGE", "FREEZE_TICK", "FREEZE_ROUND", "LAST_PAUSED"],
  ...
  CONFIG_GROUPS["Other"] = Object.keys(contestConfig).filter(
    (key) => Object.keys(CONFIG_GROUPS).map((e) => CONFIG_GROUPS[e].indexOf(key) !== -1).every((elm) => elm === false)
  );
    ...
    <div className="px-4 mb-5">
      ...
      {Object.entries(CONFIG_GROUPS).map(([group_name, keys]) => (
        <div className="my-5">
          ...
          {keys.map((elm) => (
            <div className="bg bg-neutral px-4 py-2 rounded-md grid grid-cols-7 gap-4 mb-3" key={elm}>
              <React.Fragment>
                ...
                <div className="flex flex-row items-center col-span-4 w-full">
                  {editingKey === elm ? (
                    ...
                    <div className="text-ellipsis overflow-hidden">{contestConfig[elm]}</div>
                  )}
                </div>
