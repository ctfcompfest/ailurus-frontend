...
function ServiceRow({
	...
	const restartMutation = useMutation({
		mutationFn: () =>
			postAdmin(
				`admin/teams/${team.id}/challenges/${chall.id}/service-manager/?action=restart`,
				{
					json: { confirm: true },
				},
			),
	...
	const resetMutation = useMutation({
		mutationFn: () =>
			postAdmin(
				`admin/teams/${team.id}/challenges/${chall.id}/service-manager/?action=reset`,
				{
					json: { confirm: true },
				},
			),
	...
	const deleteMutation = useMutation({
		mutationFn: () =>
			postAdmin(
				`admin/teams/${team.id}/challenges/${chall.id}/service-manager/?action=delete`,
				{
					json: { confirm: true },
				},
			),
	...
	const provisionMutation = useMutation({
		mutationFn: () =>
			postAdmin(
				`admin/teams/${team.id}/challenges/${chall.id}/service-manager/?action=provision`,
				{
					json: { confirm: true },
				},
			),
	...
	const buildImageMutation = useMutation({
		mutationFn: () =>
			postAdmin(
				`admin/teams/${team.id}/challenges/${chall.id}/service-manager/?action=build_image`,
				{
					json: { confirm: true },
				},
			),
	...
	return (
		<div className="p-4 rounded-md bg-neutral text-neutral-content flex flex-row justify-between items-center my-2">
			<div className="flex flex-col gap-1">{team.name}</div>

			<div className="flex flex-row gap-2 items-center">
				<ServiceDetailModal
					challId={chall.id}
					teamId={team.id}
					btn={<button className="btn btn-black-500 btn-sm">See detail</button>}
				/>
				<ConfirmModal
					action="Build Image"
				...
				>
					Service image for team <strong>{team.name}</strong> in challenge{" "}
					<strong>{chall.title}</strong>&nbsp; will be <u>build</u>. Are you
					sure?
				</ConfirmModal>
				<ConfirmModal
					action="Provision"
				...
				>
					Service for team <strong>{team.name}</strong> in challenge{" "}
					<strong>{chall.title}</strong>&nbsp; will be <u>provisioned</u>. Are
					you sure?
				</ConfirmModal>
				<ConfirmModal
					action="Execute"
				...
				>
					Dry run for team <strong>{team.name}</strong> in challenge{" "}
					<strong>{chall.title}</strong>&nbsp; will be <u>executed</u>. Are you
					sure?
				</ConfirmModal>
				<ConfirmModal
					action="Reset"
				...
				>
					Service for team <strong>{team.name}</strong> in challenge{" "}
					<strong>{chall.title}</strong>&nbsp; will be <u>reset</u>. Are you
					sure?
				</ConfirmModal>
				<ConfirmModal
					action="Restart"
				...
				>
					Service for team <strong>{team.name}</strong> in challenge{" "}
					<strong>{chall.title}</strong>&nbsp; will be <u>restart</u>. Are you
					sure?
				</ConfirmModal>
				<ConfirmModal
					action="Delete"
				...
				>
					Service for team <strong>{team.name}</strong> in challenge{" "}
					<strong>{chall.title}</strong>&nbsp; will be <u>deleted</u>. Are you
					sure?
				</ConfirmModal>
			</div>
		</div>
...
function ChallengeServicesSection({
	chall,
	teams,
	service_idx,
}: {
	...
	const infraProvisionMutation = useMutation({
		mutationFn: () =>
			postAdmin(
				`admin/teams/${team_master_id}/challenges/${chall.id}/service-manager/?action=provision`,
				{
					json: { confirm: true },
				},
			),
		...
		<div className="my-5">
			...
			{teams?.map((team) => (
				<ServiceRow
					...
					key={`servicerow-${team.id}-${chall.id}`}
				/>
			))}
		</div>
	);
